...
public class BoardDAO {
	...
	public List<BoardBean>  getBoardList(int startRow,int pageSize,String search){
		...
		try {
			...
			while(rs.next()){
				...
				bBean.setDatetime(rs.getTimestamp("date"));
	 ...
	 */
	 ...
	 public void reInsertBoard(BoardBean bBean){		 
		 ...
		 try {
			 ...
			 pstmt.setString(12, bBean.getRealfile());
			 //답변 INSERT
			 pstmt.executeUpdate();	 
		 } catch (Exception e) {
	...
	public BoardBean boardInfo(int num, String file){
		BoardBean boardBean = null;
		try {
			con = getConnection();
			sql = "select * from board where num=? and file=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, num);
			...
			if(rs.next()){
				...
				boardBean.setDatetime(rs.getTimestamp("date"));
				...
				boardBean.setRealfile(rs.getString("realfile"));
			}
		} catch (Exception e) {		
			System.out.println("realpath메소드 내부 오류 : "+e);
		} finally {
			//자원해제
			if(rs!=null)try{rs.close();}catch(Exception err){err.printStackTrace();}
			if(pstmt!=null)try{pstmt.close();}catch(Exception err){err.printStackTrace();}
	...
	public List<BoardBean> getBoradList(int startRow,int pageSize){
		...
		try{
			...
			while (rs.next()) {
				...
				bBean.setReadcount(rs.getInt("readcount"));
				bBean.setSubject(rs.getString("subject"));
				bBean.setRealfile(rs.getString("realfile"));
	...
	public void insertBoard(BoardBean bBean){		
		...
		try{
			...
			sql = "insert into board "
				...
				+ "re_seq,readcount,date,ip,realfile)"
				+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
			...
			pstmt.setTimestamp(11, bBean.getDatetime());//글 작성 날짜
			pstmt.setString(12, bBean.getIp());//글쓴사람의 IP주소 
			pstmt.setString(13, bBean.getRealfile());//추가할 새글 데이터중 업로드된 실제 파일명
